...
**5. Пользователь БД из FastPanel живёт только на сокете.**
Панель создаёт учётку как `user@localhost`, а это в MySQL означает **только
unix-сокет**. Prisma по умолчанию подключается по TCP на `127.0.0.1` и получает
`Access denied`, хотя `mysql -h localhost` из консоли при этом работает. Лечится
параметром `?socket=/run/mysqld/mysqld.sock` в `DATABASE_URL`. Побочный плюс:
база вообще не торчит в сеть.

Там же: спецсимволы в пароле надо URL-кодировать (`)` → `%29`, `,` → `%2C`,
`:` → `%3A`, `>` → `%3E`, `@` → `%40`).

**То же касается `mysqldump`.** С `--host/--port` он уходит по TCP и получает
`Access denied`, поэтому бэкап берёт путь из параметра `?socket=` и передаёт
`--socket`. Первые три «бэкапа» на проде оказались пустыми именно из-за этого:
ошибка глушилась через `2>/dev/null`, а проверка «файл не пустой» ничего не ловила —
**gzip от пустого потока весит ~20 байт и формально непустой**. Теперь успех
определяется по завершающей строке `Dump completed`, а в лог пишется число таблиц.

**6. Кодировка базы.** FastPanel создаёт базу в `utf8` (это трёхбайтовый
`utf8mb3` — без эмодзи). Меняется, пока база пустая:
`ALTER DATABASE ... CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`.

**7. Автозапуск PM2.** `pm2 startup` требует root, которого нет. Вместо него
в пользовательском crontab: `@reboot ... pm2 resurrect`.

**8. `npm ci` после загрузки `.env` теряет devDependencies.**
В `.env` стоит `NODE_ENV=production`, а в этом режиме npm не ставит
devDependencies — то есть `@nestjs/cli`, `tsc` и `vite`. Сборка падает с
`sh: 1: nest: not found`. Скрипт деплоя читает `.env` раньше (нужен для бэкапа
базы), поэтому установка идёт с явным `npm ci --include=dev`.

**9. Витрину раздаёт Node, а не Nginx.**
Сайт в FastPanel настроен как «Обратный прокси», и в этом режиме Nginx файлы с
диска не отдаёт вообще, а поле «Рабочая директория» в панели заблокировано.
Поэтому статику раздаёт сам API через `ServeStaticModule`, а в панели указан
единственный адрес `http://127.0.0.1:4000`. Побочный плюс: конфигурация лежит
в репозитории и едет вместе с деплоем, панель её не перетирает.

**10. Smoke обязан проверять `Content-Type`.** Если Nginx настроен неверно и отдаёт
